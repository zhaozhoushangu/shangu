...
// 2. 地图核心逻辑：石家庄地图+赵州标黄（调整初始缩放为0.8并居中）
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 初始化ECharts实例（确保容器已渲染完成）
    setTimeout(() => {
        let myChart = echarts.init(mapContainer);
        
        // 核心配置：初始缩放0.8，固定石家庄中心坐标（确保地图居中）
        const SHIJIAZHUANG_CENTER = [114.54, 38.02]; // 石家庄地理中心，确保地图居中
        const SHIJIAZHUANG_INIT_ZOOM = 0.8; // 初始缩放比例改为0.8
            ...
            try {
                ...
                const option = {
                    ...
                    visualMap: {
                        ...
                        max: 1,
                        inRange: {
                            color: ['#E0E0E0', 'rgba(255, 235, 59, 0.8)']
                        }
                    },
                    series: [
                        {
                            ...
                            map: '石家庄市',
                            roam: true, // 允许后续缩放和平移
                            zoom: SHIJIAZHUANG_INIT_ZOOM, // 初始缩放设为0.8
                            center: SHIJIAZHUANG_CENTER, // 强制居中于石家庄
                            scaleLimit: { min: 0.5, max: 10 }, // 放宽最小缩放限制
                            label: {
                                show: true,
                                fontSize: 12,
                                color: '#3E2723'
                ...
                window.addEventListener('resize', function() {
                    ...
                    myChart.setOption({
                        series: [{
                            zoom: myChart.getOption().series[0].zoom || SHIJIAZHUANG_INIT_ZOOM,
